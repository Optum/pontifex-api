...
import { generateHandler } from "./handlers/getPendingPermissionRequest"

const handler = async function (context: AuthenticatedContext, req: HttpRequest) {
    context.log("Generating and calling handler for PermissionRequestGetPending")
    context.log("user JWT", JSON.stringify(context.jwtToken))
